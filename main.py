...
def plot_event_map(strmacc1,strmori):
    ...
    depstacks = np.zeros(ndep,)
    for idep in range(ndep):
        stackenergyp = 0
        stackenergys = 0
        for ii in range(len(strmacc1)):
            dist  = strmacc1[ii].stats.distance
...
def plot_event(strmacc1,strmori):
    ...
    ax4.set_ylim([-0.1,ymax])
    ax4.set_title('Mapping to depth_'+str(evdep1))
    fig.savefig(figdir+'/'+str(ievent)+'_'+evname1+'_Mw'+str(evmag1)+'_td.png',dpi=200)
    plt.ioff()
    plt.close()
    datasave_sub['stackenv'] = stack
    datasave_sub['sdep'] = np.vstack([ddep,depstack])
...
for ievent in range(0,nevent):
    ...
    if len(strmacc1)<50:
        continue
    datasavesub = plot_event_map(strmacc1,strmori)
